...
module.exports = function handleChoiceMade(client, data, config, io, firstTrialStartingTimeRef) {
	...
	if (choice > -1) {
		console.log(` - Client ${session} (subNo = ${number}) chose ${choice} and got ${this_indiv_payoff} at t = ${this_trial} of ${gameType}.`);
	} else {
		console.log(` - Client ${session} (subNo = ${number}) missed at t = ${this_trial} of ${gameType}.`);
	}
	...
	console.log(` - doneNo: ${room.doneNo[p]}, current round is ${room.trial} at ${client.room}`);
